...
func (m *Message) UnmarshalJSON(js []byte) error {
	return m.UnmarshalJSONPB(&jsonpb.Unmarshaler{AllowUnknownFields:true}, js)
}
...
func (m *Message) unmarshalJson(r *jsReader, opts *jsonpb.Unmarshaler) error {
	...
	for r.hasNext() {
		...
		if err != nil {
			//标注那个message的哪个field转换失败
			return fmt.Errorf("message type %s field named %s convert fail", m.md.GetFullyQualifiedName(), f)
		}
		if v != nil {
